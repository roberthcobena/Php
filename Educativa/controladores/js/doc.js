$(document).ready(function() {
    load(1);
    $('#docInfo').on('hidden.bs.modal', function(e) { //ID DEL MODAL
        $('#edita_doc').trigger("reset"); //ID DEL FORM
    })
...
function load(page) {
    ...
    $("#loader").fadeIn('slow');
    $.ajax({
        url: '../modelos/tablas/docente.php?action=ajax&page=' + page + '&q=' + q,
...
$("#edita_doc").submit(function(event) { //NOMBRE DEL FORM
    $('#guarda_Doc').attr("disabled", true); //NOMBRE DE BOTÓN QUE HACE SUBMIT    
    event.preventDefault();
    $.ajax({
        type: "POST",
        url: "../controladores/funciones/editar_doc.php", //ARCHIVO CON SQL DE EDICIÓN
        data: new FormData(this),
        contentType: false,
        cache: false,
        processData: false,
        beforeSend: function() {
            $('#guarda_Doc').attr("disabled", true); //DESACTIVAMOS BOTÓN MIENTRAS SE EJECUTA ORDEN 
            $("#resultados_ajax").html("Realizando cambios, por favor espere..."); //MENSAJES DE ESPERA, ÉXITO O ERROR
        },
        success: function(datos) {
            $("#resultados_ajax").html(datos);
            $('#guarda_Doc').attr("disabled", false);
...
function esconde_modal_edicion() {
    $('#docInfo').modal('hide'); //ESCONDEMOS MODAL SEGÚN SU NOMBRE
