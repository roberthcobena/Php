...
function load(page) {
    ...
    $("#loader").fadeIn('slow');
    $.ajax({
        url: '../../modelos/tablas/resultados_alumno.php?action=ajax&page=' + page + '&q=' + q + '&idtaller=' + idtaller,
...
$("#edita_pregu").submit(function(event) { //NOMBRE DEL FORM
    $('#guarda_pre').attr("disabled", true); //NOMBRE DE BOTÓN QUE HACE SUBMIT    
    event.preventDefault();
    $.ajax({
        type: "POST",
        url: "../../controladores/funciones/editar_pregunta.php", //ARCHIVO CON SQL DE EDICIÓN
        data: new FormData(this),
        contentType: false,
        cache: false,
        processData: false,
        beforeSend: function() {
            $('#guarda_pre').attr("disabled", true); //DESACTIVAMOS BOTÓN MIENTRAS SE EJECUTA ORDEN 
...
function esconde_modal_edicion() {
    $('#preguInfo').modal('hide'); //ESCONDEMOS MODAL SEGÚN SU NOMBRE
