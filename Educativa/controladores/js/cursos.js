...
function reporteid(id) {
    var id = $("#id" + id).val(); //asignamos el input escondido de la tabla a una variable Js

    $("#n_id").val(id); //Convertimos la variable Js a contenido legible para el modal
}

function captura(){
    var cId=document.getElementById("n_id").value;
    imprimir_reporte(cId);
}
...
function imprimir_reporte(id) {
    VentanaCentrada('../recursos/pdf/Promedios/ver-reporte.php?id_curso=' + id, 'Taller', '', '1024', '768', 'true');
}

function imprimir_excel(id) {
    location.href ='../recursos/excel/promedio_Estudiantil.php?id_curso=' + id ;
}
