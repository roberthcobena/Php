<?php
	if (empty($_POST['m_id'])) { //validamos que no este vacío el campo
       $errors[] = "Falta id del estudiante"; //mensaje en caso de error
    }else if (empty($_POST['m_nombre'])) { 
        $errors[] = "Falta nombre del estudiante";
    }else if (empty($_POST['m_apellido'])) { 
        $errors[] = "Falta apellido del estudiante";
		...
		$sql="UPDATE usuario u
				...
				SET d.nomb_user='$nombre', d.apell_user='$apellido', a.alum_seccion='$curso', a.alum_estado='$estado', u.u_estado='$estado', d.esta_user='$estado' WHERE a.alum_id='$id_user'";
