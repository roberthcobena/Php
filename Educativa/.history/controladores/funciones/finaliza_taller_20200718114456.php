	   ...
	   $errors[] = "Conteste la pregunta 10";
		...
		$pre10 = intval($_POST['pregu10']);		
					...
					if ($query1 AND $query2 AND $query3 AND $query4 AND $query5 AND $query6 AND $query7 AND $query8 AND $query9 AND $query10) {
						...
						$sqlr = "SELECT * FROM pregunta p WHERE p.id_pregunta=$pre_c";
							$queryr = mysqli_query($con, $sqlr);
							while ($rowr = mysqli_fetch_array($queryr)) {
								if ($resp1 == $rowr['opc_correcta']) {
									++$nota;
									$pre_c=$pre2;
								} 				
								$queryr = mysqli_query($con, $sqlr);
								while ($rowr = mysqli_fetch_array($queryr)) {
									if ($resp2 == $rowr['opc_correcta']) {
										++$nota;
										$pre_c=$pre3;
									} 				
									$queryr = mysqli_query($con, $sqlr);
									while ($rowr = mysqli_fetch_array($queryr)) {
										if ($resp3 == $rowr['opc_correcta']) {
											++$nota;
											$pre_c=$pre4;
										} 				
										$queryr = mysqli_query($con, $sqlr);
										while ($rowr = mysqli_fetch_array($queryr)) {
											if ($resp4 == $rowr['opc_correcta']) {
												++$nota;
												$pre_c=$pre5;
											} 				
											$queryr = mysqli_query($con, $sqlr);
											while ($rowr = mysqli_fetch_array($queryr)) {
												if ($resp5 == $rowr['opc_correcta']) {
													++$nota;
													$pre_c=$pre6;
												} 				
												$queryr = mysqli_query($con, $sqlr);
												while ($rowr = mysqli_fetch_array($queryr)) {
													if ($resp6 == $rowr['opc_correcta']) {
														++$nota;
														$pre_c=$pre7;
													} 				
													$queryr = mysqli_query($con, $sqlr);
													while ($rowr = mysqli_fetch_array($queryr)) {
														if ($resp7 == $rowr['opc_correcta']) {
															++$nota;
															$pre_c=$pre8;
														} 				
														$queryr = mysqli_query($con, $sqlr);
														while ($rowr = mysqli_fetch_array($queryr)) {
															if ($resp8 == $rowr['opc_correcta']) {
																++$nota;
																$pre_c=$pre9;
															} 				
															$queryr = mysqli_query($con, $sqlr);
															while ($rowr = mysqli_fetch_array($queryr)) {
																if ($resp9 == $rowr['opc_correcta']) {
																	++$nota;
																	$pre_c=$pre10;
																} 				
																$queryr = mysqli_query($con, $sqlr);
																while ($rowr = mysqli_fetch_array($queryr)) {
																	if ($resp10 == $rowr['opc_correcta']) {
																		++$nota;
																	} 				
																}
															}
							...
							$sql="UPDATE alumno_taller SET prom_taller='".$fin_taller."', prom_taller='".$nota."', est_taller='2' WHERE id_taller='".$taller."' AND id_alumno='".$id_user."'";
