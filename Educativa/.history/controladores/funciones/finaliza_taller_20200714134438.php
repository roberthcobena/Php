	...
	if(empty($_POST['id_alumno'])) {
	   ...
	   $errors[] = "Conteste la pregunta 10";
		...
		$pre10 = intval($_POST['pregu10']);
				...
				$sql="UPDATE alumno_taller SET fin_taller='".$finaliza."', est_taller='1' WHERE id_taller='".$taller."' ";
				...
				if ($query_update) {
					...
					if ($query1 AND $query2 AND $query3 AND $query4 AND $query5 AND $query6 AND $query7 AND $query8 AND $query9 AND $query10) {
						//Cambiamos estado del taller - PARA EL ALUMNO
						$sql="UPDATE alumno_taller SET fin_taller='".$contra."' WHERE id_usuario='".$id_user."' ";
						$query_update = mysqli_query($con,$sql);
