...
//REALIZAR TALLER
//CIERRE DEL PROYECTO 
function reporte_final(id_proyecto) {
    event.preventDefault();
    if (confirm("¿Está seguro que desea cerrar la planificación y enviar el proyecto a revisión? *SE GENERARÁ EL INFORME FINAL DE SEGUIMIENTO (Recuerde guardarlo)")) {
        $.ajax({
            type: "POST",
            url: "ajax/cierra_proy.php",
            data: "id=" + id_proyecto,
            beforeSend: function() {
                $('#guardar_cierre_f').attr("disabled", true);
                $("#resultados_cierre_f").html("Finalizando seguimiento...");
            },
            success: function(datos) {
                $("#resultados_cierre_f").html(datos);
                VentanaCentrada('./pdf/documentos/informe_final.php?id_proyecto=' + id_proyecto, 'Informe', '', '1024', '768', 'true');
                $('#guardar_cierre_f').attr("disabled", false);
