<?php    
	...
	if($action == 'ajax'){
        ...
        $Table = "respuesta r, pregunta p, alumnos a, usuario u, datos_usuarios du, alumno_taller at, taller t";
		$Where = " WHERE p.est_pregu!='' AND p.id_taller=$id_taller AND p.id_pregunta=r.id_pregunta AND r.id_alumno=a.alum_id AND a.alum_usu=u.id_usuario AND u.id_usuario=du.id_usuario AND at.id_taller=p.id_taller AND at.id_alumno=u.id_usuario AND t.id_taller = p.id_taller";
...
<table id="zero_config" class="table table-striped table-bordered text-left">
    <thead>
        <tr>
            <th colspan="8" class="bg bg-cyan">Calificaciones generales</th>            
        </tr>
        <tr>
            <th>#</th>
            <th>Estudiante</th>
    ...
    <?php
        ...
        <tr>
            ...
            <td style="text-align: center;">
                <?php
                    // while($row_p= mysqli_fetch_array($query2)){ 
                        //     $sql_aciertos   = mysqli_query($con, "SELECT count(*) AS numrows FROM pregunta p, respuesta r WHERE  p.id_taller=" . $row_p['id_taller'] . " AND r.id_alumno=" . $row_e['alum_id'] . " AND r.resp_alumno=" . $row_p['opc_correcta'] . " AND r.id_pregunta=p.id_pregunta");                       
                        //     $row_a=mysqli_fetch_array($sql_aciertos);
                        //     $aciertos = $row_a['numrows'];                 
                        // }
                        // echo $aciertos. "/" . $numpreguntas;
                        
                    $aciertos=intval('');
                    while ($row_p= mysqli_fetch_array($query2)){
                        echo $taller_r=$row_p['id_taller'];
                        echo $correcta_r=$row_p['opc_correcta'];
                        $sql_aciertos   = mysqli_query($con, "SELECT count(*) AS numrows FROM pregunta p, respuesta r WHERE  p.id_taller=" . $taller_r . " AND r.id_alumno=" . $row_e['alum_id'] . " AND r.resp_alumno=" . $correcta_r . " AND r.id_pregunta=p.id_pregunta");                       
                        if ($result = mysqli_query($con, $sql2)) {
                            $row_cnt = mysqli_num_rows($result);
                            if ($row_cnt > 0) {
                                $query3 = mysqli_query($con, $sql_aciertos);
                                while ($row_a = mysqli_fetch_array($query3)) {
                                    $aciertos=$row_a['numrows'];
                                    printf("<b>" . $aciertos . "/" . $numpreguntas . "</b>\n");
                                }
                            } else {
                                printf("<b>" . $aciertos . "</b>\n", $row_cnt);
                            }
