        ...
        <tr>
            ...
            <td style="text-align: center;">
                <?php
                    ...
                    $aciertos=intval('');
                    $row_p= mysqli_fetch_array($query2);              
                    echo$sql_aciertos   = mysqli_query($con, "SELECT count(*) AS numrows FROM pregunta p, respuesta r WHERE  p.id_taller=" . $row_p['id_taller'] . " AND r.id_alumno=" . $row_e['alum_id'] . " AND r.resp_alumno=" . $row_p['opc_correcta'] . " AND r.id_pregunta=p.id_pregunta");                       
                    if ($result = mysqli_query($con, $sql2)) {
                        $row_cnt = mysqli_num_rows($result);
                        if ($row_cnt > 0) {
                            $query3 = mysqli_query($con, $sql_aciertos);
                            while ($row_a = mysqli_fetch_array($query3)) {
                                printf($row_a['numrows'] . "/" . $numpreguntas . "");
                            }
                        } else {
                            printf("<b>" . $aciertos . "</b>\n", $row_cnt);
                        }
