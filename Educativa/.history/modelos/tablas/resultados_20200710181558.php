    ...
    <?php
            ...
            while ($row_e= mysqli_fetch_array($query)){
                ...
                while($row_p= mysqli_fetch_array($query2)){ 
                    $sql_aciertos   = mysqli_query($con, "SELECT count(*) AS numrows FROM respuesta r, pregunta p WHERE  r.id_alumno=" . $row_e['alum_id'] . " AND r.id_pregunta=p.id_pregunta AND p.id_taller=" . $row_p['id_taller'] . " AND r.resp_alumno=" . $row_p['opc_correcta']);                       
