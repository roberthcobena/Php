<?php    
    ...
    include ("../../controladores/rol-usuario.php");//Contiene validaciones segun rol de usuario

    
    $action = (isset($_REQUEST['action'])&& $_REQUEST['action'] !=NULL)?$_REQUEST['action']:'';
	if($action == 'ajax'){
        $q = mysqli_real_escape_string($con,(strip_tags($_REQUEST['q'], ENT_QUOTES)));
	    $sTable = "contratistas c";
		$sWhere = "";
		$sWhere.=" WHERE c.est_contatis!=''";
	if ( $_GET['q'] != "" ){
        $sWhere.= " AND  (c.nom_contra LIKE '%$q%')";
    }
        $sWhere.=" ORDER BY c.nom_contra DESC ";
        include '../../controladores/pagination.php';	
