...
function funcionandox() {
    var fechax = $("#inicia").val();
    var iniciox = new Date(fechax);
    // obtenemos la fecha actual
    var actualx = new Date().getTime();
    // obtenemos la diferencia entre la fecha actual y la de inicio
    var diffx = new Date(actualx - iniciox);
    var resultx = LeadingZerox(diffx.getUTCMinutes());  
    // Indicamos que se ejecute esta función nuevamente dentro de 1 segundo
    console.log(resultx);
    timeoutx = setTimeout("funcionandox()", 1000);
    //indicamos minutos
    if (resultx == 3) {
        tiempolimite()
    }
...
function LeadingZerox(Timex) {
    return (Timex < 10) ? "0" + Timex : +Timex;
}
