        ...
        <tbody>
            <?php
                ...
                while ($row2= mysqli_fetch_array($query)){
                    ++$contador2;
                    //ids
                    $id2=$row2['id_pregunta'];
                    //nombres
                    $pregunta2=$row2['des_pregunta'];
                    $op12=$row2['opc_1'];
                    $op22=$row2['opc_2'];
                    $op32=$row2['opc_3'];
                    $op42=$row2['opc_4'];
                    $correcta2=$row2['opc_correcta'];
                    $estado2=$row2['est_pregu'];

                    //color estado
                    if ($estado2 == 1) {
                        $estado_color2 = "success";
                    } elseif ($estado2 == 2) {
                        $estado_color2 = "danger";
                    }
                    //para mensaje por estado                                        
                    if ($estado2 == 1) {
                        $msg_estado2 = "Activo";
                    } elseif ($estado2 == 2) {
                        $msg_estado2 = "Inactivo";
                    }if($estado2)                   
                    ?>
                <input type="hidden" value="<?php echo $id2;?>" id="id<?php echo $id2;?>">    
                <input type="hidden" value="<?php echo $id_taller2;?>" id="id_taller<?php echo $id2;?>">  
                <input type="hidden" value="<?php echo $pregunta2;?>" id="pregunta<?php echo $id2;?>">  
                <input type="hidden" value="<?php echo $op12;?>" id="op1<?php echo $id2;?>">  
                <input type="hidden" value="<?php echo $op22;?>" id="op2<?php echo $id2;?>">
                <input type="hidden" value="<?php echo $op32;?>" id="op3<?php echo $id2;?>">
                <input type="hidden" value="<?php echo $op42;?>" id="op4<?php echo $id2;?>">
                <input type="hidden" value="<?php echo $correcta2;?>" id="correcta<?php echo $id2;?>">    
                <input type="hidden" value="<?php echo $estado2;?>" id="estado<?php echo $id2;?>">    
            <tr>
                ...
                <td><?php echo $op32;?></td>
                <td><?php echo $op42;?></td> 
                <td><?php echo "Opcion: ".$correcta2;?></td>         
                <td><span class="label label-<?php echo $estado_color2; ?>"><?php echo $msg_estado2; ?></span> </td>
                <td style="text-align: center; white-space: nowrap;">
                    <button type="button" class="btn btn-success" title='Detalles' onclick="detalles('<?php echo $id2;?>');" data-toggle="modal" data-target="#preguInfo"><i class="nav-icon fas fa-edit"></i> Editar</button>                    
                </td>       
