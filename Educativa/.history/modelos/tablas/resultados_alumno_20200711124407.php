...
<table id="zero_config" class="table table-striped table-bordered text-left">
    ...
    ?>
        <tr>
            ...
            <td><?php 
                $correcto="";
                $sql2 = "SELECT * FROM respuesta r, alumnos a WHERE r.id_alumno=a.alum_id AND a.alum_usu = " . $id_al . " AND r.id_pregunta= " . $row_e['id_pregunta'] ." AND r.resp_alumno= " . $row_e['opc_correcta'] ." ";
                if ($result = mysqli_query($con, $sql2)) {
                    $row_cnt = mysqli_num_rows($result);
                    if (empty($row_cnt)) {
                            printf("<b>" . $contes . "</b>");
