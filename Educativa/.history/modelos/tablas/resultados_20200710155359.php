<?php    
	...
	if($action == 'ajax'){
        ...
        $sTable = "respuesta r, pregunta p, alumnos a, usuario u, datos_usuarios du, alumno_taller at";
		$sWhere = " WHERE p.est_pregu!='' AND p.id_taller=$id_taller AND p.id_pregunta=r.id_pregunta AND r.id_alumno=a.alum_id AND a.alum_usu=u.id_usuario AND u.id_usuario=du.id_usuario AND at.id_taller=p.id_taller AND at.id_alumno=u.id_usuario";
        $sWhere1=" GROUP BY r.id_alumno";	
        $sWhere2=" GROUP BY p.id_pregunta";	
        $sWhere3=" ORDER BY des_pregunta ASC ";                            
        $sql="SELECT * FROM  $sTable . $sWhere . $sWhere1 . $sWhere3";
        $sql2="SELECT * FROM  $sTable . $sWhere . $sWhere2 . $sWhere3";
