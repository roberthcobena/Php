<?php    
	...
	if($action == 'ajax'){
		...
		$sWhere = "";
		$sWhere.=" WHERE t.esta_taller!='' AND u.id_unidad=e.id_unidad AND e.id_tema=t.tema_taller AND t.curs_taller=c.id_curso AND c.jornada=j.id_seccion AND c.id_curso=d.id_curso AND d.id_docente=o.id_docente AND o.id_usuario=$id_docen AND c.id_curso=$id_curso";
	if ( $_GET['q'] != "" ){
    ...
    <table id="zero_config" class="table table-striped table-bordered text-center">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><a href="#" class='btn btn-default' title='Detalles del taller' onclick="detalles('<?php echo $id;?>');" data-toggle="modal" data-target="#talInfo"><strong><i><?php echo $taller; ?></i></strong></td>
                ...
                <td style="text-align: center; white-space: nowrap;">
                    <?php
                        if ($estado==1) {                        
                    ?>
                        <a href="gestion_taller.php?id_taller=<?php echo $id;?>" class='btn btn-success' title='Gestionar taller'><i class="glyphicon glyphicon-edit"> Gestionar</i></a>
                        <!-- <button type="button" class="btn btn-success" title='Detalles' onclick="gestion('<?php echo $id;?>');" ><i class="nav-icon fas fa-edit"></i> Gestionar </button>                     -->
                    <?php
                        } elseif ($estado==2) {
                    ?>
                        <button type="button" class="btn btn-danger" title='Detalles' onclick="resultados('<?php echo $id;?>');" ><i class="nav-icon fas fa-check"></i> Ver resultados</button>                        
                    <?php
                        }
                    ?>
                </td>
