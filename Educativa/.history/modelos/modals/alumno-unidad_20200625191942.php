            ...
            <div class="modal-body">
                    ...
                    <label for="m_n_unidad" class="col-sm-2 text-left control-label col-form-label">Talleres: </label>
                        <table id="zero_config" class="table table-striped table-bordered text-center">
                            ...
                            <?php 
                                ...
                                $contador_t=0;
                                $sql_t = "SELECT * from cursos c, taller t, tema te, unidad u
                                          WHERE c.est_curso!='' 
                                          AND t.esta_taller!='' 
                                          AND  t.curs_taller=c.id_curso
                                          AND t.tema_taller=te.id_tema
                                          AND te.Id_unidad=u.id_unidad
                                          ORDER BY t.inicio_t DESC";
                                $query_t = mysqli_query($con, $sql_t);
                                while ($row_t = mysqli_fetch_array($query_t)) {
                                    ++$contador_t; 
                                    $id_taller=$row_t['id_taller'];
                                    $n_taller=$row_t['nom_taller'];
                                    $tema=$row_t['te_nombre'];
                ...
                ?>
                        ...
                        <div class="modal-footer">                            
                            <button type="submit" class="btn btn-primary" id="ver_Taller" title="Ver reporte" onclick="reporte('<?php echo $id_unidad;?>');">Reporte</button>
                        </div>                   
                </form>
        </div>
    </div>
