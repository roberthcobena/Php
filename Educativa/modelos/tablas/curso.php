<?php    
    require_once ("../../controladores/conexion/db.php");
    require_once ("../../controladores/conexion/conexion.php");    
    
    $action = (isset($_REQUEST['action'])&& $_REQUEST['action'] !=NULL)?$_REQUEST['action']:'';
	if($action == 'ajax'){
        $q = mysqli_real_escape_string($con,(strip_tags($_REQUEST['q'], ENT_QUOTES)));
	    $sTable = "cursos c, jornadas j, anios_lectivos a";
		$sWhere = "";
		$sWhere.=" WHERE c.est_curso!='' AND c.jornada=j.id_seccion AND c.an_lec=a.id";
	if ( $_GET['q'] != "" ){
        $sWhere.= " AND  (c.nom_curso LIKE '%$q%' OR c.cod_curso LIKE '%$q%')";
    }
        $sWhere.=" ORDER BY c.cod_curso ASC ";
        include 'pagination.php';	
        $page = (isset($_REQUEST['page']) && !empty($_REQUEST['page']))?$_REQUEST['page']:1;
        $per_page = 20;
        $adjacents  = 4;
        $offset = ($page - 1) * $per_page;    
        $count_query   = mysqli_query($con, "SELECT count(*) AS numrows FROM $sTable  $sWhere");
        $row= mysqli_fetch_array($count_query);
        $numrows = $row['numrows'];
        $total_pages = ceil($numrows/$per_page);
        $reload = '#';    
        $sql="SELECT * FROM  $sTable $sWhere LIMIT $offset,$per_page";
        $query = mysqli_query($con, $sql);    
        if ($numrows>0){
            echo mysqli_error($con);
            $resultado=" Cursos encontrados";
?>
<center><p><b><?php echo $numrows; ?></b><?php echo $resultado; ?></p>       </center>

<div class="card-body table-responsive p-0">
    <table id="zero_config" class="table table-striped table-bordered text-center">
        <thead>
            <tr>
                <th>#</th>
                <th>Código</th>
                <th>Curso</th>
                <th style="text-align: center;">Sección</th>
                <th style="text-align: center;">Jornada</th>
                <th style="text-align: center;">Año lectivo</th>
                <th style="text-align: center;">Estado</th>
                <th style="text-align: center;">Opciones</th>
            </tr>
        </thead>
        
        <tbody>
            <?php
                $contador=0;
                $estado_color="";
                $msg_estado = "";
                while ($row=mysqli_fetch_array($query)){
                    ++$contador;
                    $id=$row['id_curso'];
                    $cod=$row['cod_curso'];
                    $curso=$row['nom_curso'];
                    $año=$row['anio'];
                    $id_año=$row['id'];
                    $seccion=$row['sec_curso'];
                    $paralelo=$row['nom_seccion'];
                    $jornada=$row['jornada'];
                    $estado=$row['est_curso'];

                    $id_curso=$row['id_curso']; 
                    
                    //para colores por estado                    
                    if ($estado == 1) {
                        $estado_color = "success";
                    } elseif ($estado == 2) {
                        $estado_color = "danger";
                    }elseif ($estado == 3) {
                        $estado_color = "primary";
                    }
                    //para mensaje por estado                                        
                    if ($estado == 1) {
                        $msg_estado = "Activo";
                    } elseif ($estado == 2) {
                        $msg_estado = "Inactivo";
                    }elseif ($estado == 3) {
                        $msg_estado = "Finalizado";
                    }
                    ?>
                <input type="hidden" value="<?php echo $id;?>" id="id<?php echo $id;?>">	
                <input type="hidden" value="<?php echo $cod;?>" id="cod<?php echo $id;?>">	
                <input type="hidden" value="<?php echo $curso;?>" id="curso<?php echo $id;?>">	
                <input type="hidden" value="<?php echo $seccion;?>" id="seccion<?php echo $id;?>">	
                <input type="hidden" value="<?php echo $jornada;?>" id="jornada<?php echo $id;?>">	
                <input type="hidden" value="<?php echo $id_año;?>" id="año<?php echo $id;?>">	
                <input type="hidden" value="<?php echo $estado;?>" id="estado<?php echo $id;?>">	
            <tr>
                <td><?php echo $contador;?></td>
                <td><?php echo $cod;?></td> 
                <td><?php echo $curso;?></td>        
                <td><?php echo $seccion;?></td>        
                <td><?php echo $paralelo;?></td>        
                <td><?php echo $año;?></td>        
                <td><span class="label label-<?php echo $estado_color; ?>"><?php echo $msg_estado; ?></span></td>

                <?php
                if ($estado == 1 || $estado == 2) {
                ?>
                    <td style="text-align: center; white-space: nowrap;">
                    <button type="button" class="btn btn-success" title='Detalles' onclick="detalles('<?php echo $id;?>');" data-toggle="modal" data-target="#cursoInfo"><i class="nav-icon fas fa-edit"></i> Editar</button>                    
                    </td>
                <?php
                }elseif ($estado == 3) {
                ?>
                    <td style="text-align: center; white-space: nowrap;">
                      <!--   CON MODALS -->
                    <button type="button" class="btn btn-primary" title='Detalles' data-toggle="modal" onclick="reporteid('<?php echo $id;?>');" data-target="#reporte"><i class="nav-icon fas fa-book"></i> Ver reportes</button>
                     <!--   CON MODALS -->                 


                    </td>
                <?php
                }
                ?>     
                
            </tr>
            <?php
                }
                ?>            
                <tr>
                    <td colspan=8><span class="pull-right">
                    <?php
                     echo paginate($reload, $page, $total_pages, $adjacents);
                    ?></span></td>
                </tr>
        </tbody>
    </table>    
</div>
<?php
}
}
?>