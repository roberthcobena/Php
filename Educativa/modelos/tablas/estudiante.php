<?php    
    ...
    if($action == 'ajax'){
        ...
        $sWhere = "";
        $sWhere.=" WHERE u.u_estado!=''  AND u.id_usuario=d.id_usuario AND u.id_usuario=a.alum_usu AND c.id_curso=a.alum_seccion AND  c.jornada=j.id_seccion ";
    if ( $_GET['q'] != "" ){
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?php //cursos del estudiante
                    $sql3 = "SELECT * FROM cursos c, alumnos a WHERE a.alum_usu = " . $row['id_usuario'] . " AND a.alum_seccion=c.id_curso";
                    if ($result = mysqli_query($con, $sql3)) {
                        $row_cnt = mysqli_num_rows($result);
                        if ($row_cnt > 0) {
                            $query3 = mysqli_query($con, $sql3);
                            while ($row_cursos = mysqli_fetch_array($query3)) {
                                printf($row_cursos['nom_curso'] . "; ");
                            }
                        } else {
                            printf("<b>No se encuentra matriculado en ningún curso</b>\n", $row_cnt);
                        }
                    }                    
                ?></td> 
